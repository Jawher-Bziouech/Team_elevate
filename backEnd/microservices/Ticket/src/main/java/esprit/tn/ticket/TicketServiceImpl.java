...
public class TicketServiceImpl implements TicketService {
    ...
    public TicketResponse createTicket(TicketRequest request) {
        ...
        Ticket savedTicket = ticketRepository.save(ticket);
        log.info("Ticket créé avec succès: {}", savedTicket.getTicketId());

        return mapToResponse(savedTicket);
    }

    @Override
    public TicketResponse rateTicket(Long ticketId, Long userId, String userRole, RatingRequest ratingRequest) {
        Ticket ticket = ticketRepository.findById(ticketId)
                .orElseThrow(() -> new RuntimeException("Ticket non trouvé"));

        // Vérifier que c'est le créateur du ticket
        if (!ticket.getCreatedBy().equals(userId)) {
            throw new RuntimeException("Seul le créateur du ticket peut l'évaluer");
        }

        // Vérifier que le ticket peut être évalué (résolu ou fermé)
        if (ticket.getStatus() != TicketStatus.RESOLVED && ticket.getStatus() != TicketStatus.CLOSED) {
            throw new RuntimeException("Le ticket doit être résolu ou fermé pour être évalué");
        }

        // Vérifier la note (1-5)
        ...
        ticket.setRatingComment(ratingRequest.getComment());
        ticket.setRatingDate(new Date());

        Ticket updatedTicket = ticketRepository.save(ticket);
        log.info("Ticket {} évalué avec {} étoiles", ticketId, rating);
    ...
    public TicketResponse respondToTicket(Long ticketId, Long adminId, String adminName, String response) {
        ...
        Ticket updatedTicket = ticketRepository.save(ticket);
        log.info("Réponse ajoutée au ticket: {}", ticketId);
    ...
    public TicketResponse resolveTicket(Long ticketId, Long userId, String userRole, String resolution) {
        ...
        Ticket updatedTicket = ticketRepository.save(ticket);
        log.info("Ticket résolu: {}", ticketId);
    ...
    public TicketResponse closeTicket(Long ticketId, Long userId, String userRole) {
        ...
        Ticket updatedTicket = ticketRepository.save(ticket);
        log.info("Ticket fermé: {}", ticketId);
    ...
    private TicketResponse mapToResponse(Ticket ticket) {
        ...
        // NOUVEAU: Ajouter les informations d'évaluation
        response.setRating(ticket.getRating());
        response.setRatingComment(ticket.getRatingComment());
        response.setRatingDate(ticket.getRatingDate());
        response.setCanBeRated(ticket.getStatus() == TicketStatus.RESOLVED || ticket.getStatus() == TicketStatus.CLOSED);
